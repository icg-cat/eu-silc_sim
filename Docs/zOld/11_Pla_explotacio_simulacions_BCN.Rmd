---
title: "Explotació resultats simulació - BCN"
...
Tots els resultats d'aquest document fan referència a la simulació amb dades per la ciutat de Barcelona. Les variables de rendes contemplen la transferència del IMV per les llars aplicables. 
...
ruta <- here::here("Docs", "11_taules_pla_explt_BCN.xlsx")
...
Renda anual neta mitjana per llar, persona i unitat de consum (euros). Barcelona, 2016-2017, 2017-2018, 2018-2019 i estimacions 2020. 
...
### Taula 2.2 
Taula 1: 
taula 2 amb primera columna que sigui mitjana dels IE sense ERTO; segona columna mitjana IE sense IMV; mitjana IE sense atur. Agrupat per decils 
...
df$COV_prest_atur <- ifelse(
   df$COV_select_cobra,
   df$COV_RendInd_reestimat_atSI - df$COV_RendInd_reestimat_atNO,
   0
)

df$COV_prest_erto <- ifelse(
   df$COV_select_erto, 
   df$COV_RendInd_reestimat_erto - (df$COV_import_mensual_treball * df$COV_dif_mesos_ert), 
   0
)
...
df %>% 
   ...
   summarise(
      ...
      total_ertos_cobrat_M = sum(COV_prest_erto * RB050)/1000000,
      total_atur_cobrat_M = sum(COV_prest_atur * RB050)/1000000
      ) %>% 
   gather(., "Var", "Val", -deciles_orig3) %>% 
   spread(deciles_orig3, "Val")
...
```{r eval=FALSE, include=FALSE}
do %>% 
   filter(CAMB %in% c("Barcelona")) %>% 
   group_by(HX060_R1) %>% 
   count(TRP60_Barcelona, wt = RB050) %>% 
   mutate(pp = n/sum(n)) %>% 
   filter(TRP60_Barcelona == "Pobre")
