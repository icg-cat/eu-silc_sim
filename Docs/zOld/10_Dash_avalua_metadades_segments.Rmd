...
```{r a}
ruta1 <- paste0(here::here(), "/", params$carpeta, "/", "objects")

dt <- readRDS(paste0(ruta1, "/",  "F4_dd_dashb"))
md <- readRDS(paste0(ruta1, "/",  "F3_resmat"))
param <- readRDS(paste0(ruta1, "/",  "parametres"))
...
esp <- readRDS(paste0(ruta1, "/",  "", "F1_ERTO_sect_ESP"))
...
dt %>% 
  ...
  labs(
    title = "Distribució de la durada dels episodis d'ERTO/atur", 
    y = "ecd"
  )
```
...
ATR_proj <- readRDS(paste0(ruta1, "/",  "F1_TGatur"))* 100
...
```{r cobertura}
COB_proj <- readRDS(paste0(ruta1, "/",  "F1_TGcob"))* 100
