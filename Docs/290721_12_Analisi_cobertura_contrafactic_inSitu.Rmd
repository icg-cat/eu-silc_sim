...
```{r dades}
# En referència a un directori ####

U5 <- readRDS(here::here("Outputs/210622_U5-ESP___100000_TRUE/_targets/objects/F4_dd_dashb"))
U6 <- readRDS(here::here("Outputs/210622_U5-ESP___100000_TRUE/_targets/objects/F5_contrafactic"))
newvars <- setdiff(names(U6), names(U5))
U5[,newvars] <- U6[match(U5$ID, U6$ID), newvars]

param <- readRDS(here::here("Outputs/210622_U5-ESP___100000_TRUE/_targets/objects/parametres"))
...
# U5 <- readRDS(paste0(params$wd, "/_targets/objects/F4_dd_dashb"))
# U6 <- readRDS(paste0(params$wd, "/_targets/objects/F5_contrafactic"))
# newvars <- setdiff(names(U6), names(U5))
# U5[,newvars] <- U6[match(U5$ID, U6$ID), newvars]
# 
# param <- readRDS(paste0(params$wd, "/_targets/objects/parametres"))
...
# Taules 1: IE segons deciles originals (pre 00) {.tabset}

## Taula 1.1:

**Taula 1.1:**

Mitjana dels ie_orig segons deciles (deciles **originals EMCV/ECV**), sense IMV, sense Ertos i sense atur. 
...
Mitjana dels ie_orig segons quintils (quintils **originals EMCV/ECV**), sense IMV, sense Ertos i sense atur. 

```{r}
U5$quintils <- factor(IERMB::fes_WQnt(
                        ie  = U5$ie_orig, 
                        pes = U5$RB050, 
                        ps  = seq(0,1, .2)), 
                        labels = c(1:5))
...
Mitjana dels ie_orig segons ESEeC, sense IMV, sense Ertos i sense atur. 
...
# Taules 2: IE segons deciles post-sim (pre 00) {.tabset}
...
U5 %>% 
   ...
   summarise(IE_inicial = Hmisc::wtd.mean(ie_orig, RB050), 
             simulat = Hmisc::wtd.mean(COV_IE_IMV, RB050), 
             simulat_preIMV = Hmisc::wtd.mean(COV_IE, RB050), 
             contrafactic1 = Hmisc::wtd.mean(CF1_IE, RB050), 
             contrafactic0 = Hmisc::wtd.mean(CF0_IE, RB050)
...
Mitjana dels IE segons quintils (quintils **després de simulació**), sense IMV, sense Ertos i sense atur (pre 00). 

```{r}
U5$quint_sim <- factor(IERMB::fes_WQnt(ie  = U5$COV_IE_IMV, 
                       pes = U5$RB050, 
                       ps  = seq(0,1, .2)), 
                     labels = c(1:5))
...
Mitjana dels IE segons ESeC, sense IMV, sense Ertos i sense atur (pre 00). 
