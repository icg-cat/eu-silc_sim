...
df <- readRDS(paste0(params$wd, "/_targets/objects/F4_dd_dashb"))
...
ref <- stringr::str_locate(params$wd, "Outputs")
rut <- stringr::str_sub(params$wd, end = (ref[1,1]-1))
ruta <- paste0(
   rut, 
   "Docs/",
...
```{r peces_taula_ID}
obj_files <- list.files(paste0(params$wd, "/_targets/objects/"))
...
atr <- obj_files[grep("F1_TGatur", obj_files)][1]
atur   <- readRDS(paste0(params$wd, "/_targets/objects/", atr)) %>% 
   round(., 3) %>%  as.double(.) * 100
erto   <- (as.double(param$val[param$param == "Taxa_erto_proj"]) * 100) %>% round(.,1)
prop   <- readRDS(paste0(params$wd, "/_targets/objects/F1_TGcob"))  %>% 
   round(.,3) %>% as.double(.) * 100
...
med <- Hmisc::wtd.quantile(x = df$IE, weights = df$RB050, probs = c(.5))
